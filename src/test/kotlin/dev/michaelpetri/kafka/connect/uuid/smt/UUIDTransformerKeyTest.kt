...
class UUIDTransformerKeyTest {
    ...
    private val valueTransformer: UUIDTransformer<SourceRecord> = UUIDTransformer.Key()

    @Test
    fun `uuid key gets converted from String to String (unstructured)`() {
    ...
    @Test
    fun `uuid key gets converted from String to ByteBuffer (unstructured)`() {
        val record = makeUnstructuredRecord(uuidString)
    ...
    @Test
    fun `uuid key gets converted from ByteBuffer to String (unstructured)`() {
    ...
    @Test
    fun `uuid key gets converted from String to String (structured, schemaless)`() {
        ...
        assertEquals(
            uuidString,
            (transformedRecord.key() as Map<*, *>)["uuid"]
        )
    }

    @Test
    fun `uuid key gets converted from String to ByteBuffer (structured, schemaless)`() {
        val record = makeStructuredRecordWithoutSchema(
            "uuid" to uuidString
        )
    ...
    @Test
    fun `uuid key gets converted from ByteBuffer to String (structured, schemaless)`() {
        val record = makeStructuredRecordWithoutSchema(
    ...
    @Test
    fun `uuid key gets converted from String to String (structured, with schema)`() {
        ...
        assertEquals(
            ...
            transformedRecord.keySchema().field("uuid").schema()
        )

        assertEquals(
            uuidString,
            (transformedRecord.key() as Struct)["uuid"]
        )
    }

    @Test
    fun `uuid key gets converted from String to ByteBuffer (structured, with schema)`() {
        val record = makeStructuredRecordWithSchema(
            "uuid" to uuidString
        )
    ...
    @Test
    fun `uuid key gets converted from ByteBuffer to String (structured, with schema)`() {
        val record = makeStructuredRecordWithSchema(
            "uuid" to uuidByteBuffer
        )
    ...
    @Test
    fun `uuid key gets converted from ByteBuffer to ByteBuffer (structured, with schema)`() {
        val record = makeStructuredRecordWithSchema(
            "uuid" to uuidByteBuffer
        )
